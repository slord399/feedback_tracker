...
class SearchView(ui.View):
    ...
    def update_components(self):
        ...
        post_pub.callback = self.post_public; self.add_item(post_pub)
        post_priv = ui.Button(label="Post (Private)", style=discord.ButtonStyle.primary)
        post_priv.callback = self.post_private; self.add_item(post_priv)
    ...
    async def post_public(self, interaction: discord.Interaction): await self._do_post(interaction, False)
    async def post_private(self, interaction: discord.Interaction): await self._do_post(interaction, True)
    ...
    async def _do_post(self, interaction: discord.Interaction, ephemeral: bool):
        ...
        parts = url.split("/"); uname = parts[parts.index("p") + 1] if "p" in parts else None
    ...
    async def index_selected(self, interaction: discord.Interaction):
        ...
        if lgid:
            ...
            if any(str(g.id) == str(gid) for g in self.bot.guilds):
                await self.bot.valkey.sadd("indexed_post_urls", url); await self.bot.valkey.sadd(f"guild_indexed_posts:{gid}", url); await self.bot.valkey.sadd(f"user_indexed_posts:{interaction.user.id}", f"{int(time.time())}|{url}")
        ...
        if not self.bot.guilds: return await interaction.response.send_message("Bot is not in any servers.", ephemeral=True)
...
class MyBot(commands.Bot):
    ...
    async def update_activity(self):
        try:
            ...
            activity = discord.Activity(type=discord.ActivityType.watching, name="feedback.vrchat.com", state=f"Party Info : {int(idx)} of {int(tot)}", details="Tracking Index Progress", party={'id': 'canny', 'size': [int(idx), int(tot)]})
    ...
    async def sync_localization(self):
        ...
        try:
            base = SHEET_URL.split("/edit")[0]; gid = SHEET_URL.split("gid=")[1].split("&")[0] if "gid=" in SHEET_URL else None
                ...
                async with session.get(csv_url) as resp:
                    if resp.status == 200:
                        ...
                        if "string_name" in content:
                            ...
                            self.localizer.load(); logger.info("Localization synced."); return True
    ...
    async def on_message(self, message):
        ...
        react_chan = cfg.get("react_channel"); status_chan = cfg.get("status_channel")
        if str(message.channel.id) in [react_chan, status_chan]:
            ...
            for u in urls:
                ...
                if "canny.io" in u or "feedback.vrchat.com" in u:
                    ...
                    purge = (str(message.channel.id) == status_chan); target_cid = int(status_chan or message.channel.id)
    ...
    async def index_this_canny(self, interaction: discord.Interaction, message: discord.Message):
        ...
        if not url: return await interaction.response.send_message("No URL found in message.", ephemeral=True)
        lgid = await self.valkey.get(f"last_index_selection:{interaction.user.id}")
        if lgid:
            gid = int(lgid)
            if any(str(g.id) == str(gid) for g in self.guilds):
                await self.valkey.sadd("indexed_post_urls", url); await self.valkey.sadd(f"guild_indexed_posts:{gid}", url); await self.valkey.sadd(f"user_indexed_posts:{interaction.user.id}", f"{int(time.time())}|{url}")
                await self.valkey.set(f"next_poll:{url}", 0)
                cfg = await self.valkey.hgetall(f"guild_config:{gid}"); target_cid = int(cfg.get("status_channel") or interaction.channel_id)
                ...
                return await interaction.response.send_message("Indexed!", ephemeral=True)
        if not self.guilds: return await interaction.response.send_message("Bot is not in any servers.", ephemeral=True)
    ...
    async def check_canny_status(self, interaction: discord.Interaction, message: discord.Message):
        ...
        url = next((u for u in urls if "canny.io" in u or "feedback.vrchat.com" in u), None)
        if not url: return await interaction.response.send_message("No URL found in message.", ephemeral=True)
        await interaction.response.defer(ephemeral=False); data = await fetch_canny_data(url)
        parts = url.split("/"); uname = parts[parts.index("p") + 1] if "p" in parts else None
        post = extract_post_from_data(data, uname)
        if not post: return await interaction.followup.send("Post not found.", ephemeral=True)
        lang = "English"
        if interaction.guild_id: lang = await self.valkey.hget(f"guild_config:{interaction.guild_id}", "language") or "English"
        embed = create_canny_embed(post, lang=lang); await interaction.followup.send(embed=embed, view=create_canny_view(url))
    ...
    async def post_indexed_hour(self, interaction: discord.Interaction, message: discord.Message):
        ...
        for entry in idx:
            ...
            if now - float(ts) < 3600: urls.append(url)
        if not urls: return await interaction.response.send_message("No posts indexed in the last hour.", ephemeral=True)
        await interaction.response.defer(ephemeral=False); embeds = []
        for url in urls[:10]:
            data = await fetch_canny_data(url); parts = url.split("/"); uname = parts[parts.index("p") + 1] if "p" in parts else None
            post = extract_post_from_data(data, uname)
            if post:
                lang = "English"
                if interaction.guild_id: lang = await self.valkey.hget(f"guild_config:{interaction.guild_id}", "language") or "English"
...
async def search(interaction: discord.Interaction, visibility: str = "ephemeral"):
    ephemeral = (visibility == "ephemeral"); await interaction.response.send_message("Configure filters:", view=SearchFilterView(bot, ephemeral=ephemeral), ephemeral=True)
...
async def stats(interaction: discord.Interaction):
    await interaction.response.defer(); idx = await bot.valkey.scard("indexed_post_urls"); tot = await bot.valkey.hlen("canny_search_index")
    polled = 0; async for _ in bot.valkey.scan_iter("next_poll:*"): polled += 1
    this_month = time.strftime('%Y-%m'); status_changes = await bot.valkey.get(f"stats:status_change:{this_month}") or 0; vote_reports = await bot.valkey.get(f"stats:vote_progress:{this_month}") or 0
...
@app_commands.allowed_contexts(guilds=True); @app_commands.allowed_installs(guilds=True); @app_commands.checks.has_permissions(manage_messages=True)
async def mode(interaction: discord.Interaction, mode: str):
    await bot.valkey.hset(f"guild_config:{interaction.guild_id}", "mode", mode.lower()); await register_guild(bot.valkey, interaction.guild_id); await interaction.response.send_message(f"Mode: {mode}")
...
async def set_status_channel(interaction: discord.Interaction, channel: discord.TextChannel):
    await bot.valkey.hset(f"guild_config:{interaction.guild_id}", "status_channel", str(channel.id)); await register_guild(bot.valkey, interaction.guild_id); await interaction.response.send_message("Status channel set.")
...
async def set_react_channel(interaction: discord.Interaction, channel: discord.TextChannel):
    await bot.valkey.hset(f"guild_config:{interaction.guild_id}", "react_channel", str(channel.id)); await register_guild(bot.valkey, interaction.guild_id); await interaction.response.send_message("React channel set.")
...
async def bulk_add(interaction: discord.Interaction):
    ...
    async for msg in interaction.channel.history(limit=100):
        ...
        for u in urls:
            u = clean_url(u)
            if "canny.io" in u or "feedback.vrchat.com" in u: await bot.valkey.sadd("indexed_post_urls", u); await bot.valkey.sadd(f"guild_indexed_posts:{interaction.guild_id}", u); found += 1
...
async def set_language(interaction: discord.Interaction):
    view = ui.View(); view.add_item(LanguageSelect(bot.valkey)); await interaction.response.send_message("Select language:", view=view, ephemeral=True)

@bot.tree.command(name="update_localization", description="Sync localization from Google Sheet")
@app_commands.allowed_contexts(guilds=True); @app_commands.allowed_installs(guilds=True)
async def update_localization(interaction: discord.Interaction):
    ...
    success = await bot.sync_localization(); await interaction.response.send_message("Updated." if success else "Failed.")
...
async def test_feed(interaction: discord.Interaction, canny_url: str):
    url = clean_url(canny_url); await interaction.response.defer(ephemeral=True); data = await fetch_canny_data(url); parts = url.split("/"); uname = parts[parts.index("p") + 1] if "p" in parts else None
    post = extract_post_from_data(data, uname)
    if not post: return await interaction.followup.send("Post not found.", ephemeral=True)
    lang = "English"
    if interaction.guild_id: lang = await bot.valkey.hget(f"guild_config:{interaction.guild_id}", "language") or "English"
    embed = create_canny_embed(post, lang=lang); await interaction.followup.send(embed=embed, view=create_canny_view(url), ephemeral=True)
